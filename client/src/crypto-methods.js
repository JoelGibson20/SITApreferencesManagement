...
export function aesGenKey(){
    var key = crypto.randomBytes(32);
    var hexKey = aesjs.utils.hex.fromBytes(key);
    return (hexKey);
}

export function AESencrypt(pref, secretKey){
...
export function AESdecrypt(encPref, secretKey){
    var key = aesjs.utils.hex.toBytes(secretKey); // Convert hex secret key to bytes
...
export function testAESjs(){
    ...
    var keyBack = aesjs.utils.hex.toBytes(hexKey);
    console.log("keyback = ", keyBack);
    console.log("same: ", key == keyBack);
    ...
    var aesCtr = new aesjs.ModeOfOperation.ctr(keyBack, new aesjs.Counter(5));
